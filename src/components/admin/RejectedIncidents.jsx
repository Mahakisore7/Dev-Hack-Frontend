...
// import { getIncidentsByStatus, subscribe, moveToUnverified } from '../../data/incidents';
// import IncidentCard from '../../IncidentCard';

// const RejectedIncidents = () => {
//   const [incidents, setIncidents] = useState(getIncidentsByStatus('rejected'));

//   useEffect(() => {
//     const unsubscribe = subscribe(() => {
//       setIncidents(getIncidentsByStatus('rejected'));
//     });
//     return () => unsubscribe();
//   }, []);
...
//       <div className="mb-6">
//         <h2 className="text-2xl font-bold text-foreground mb-2">Rejected Incidents</h2>
//         <p className="text-muted-foreground">Incidents that were rejected. You can move them back for review.</p>
//       </div>

//       {incidents.length === 0 ? (
//         <div className="text-center py-16 bg-card rounded-xl border border-border">
//           <p className="text-muted-foreground text-lg">No rejected incidents</p>
//         </div>
//       ) : (
...
//                   onClick: () => handleMoveToUnverified(incident.id),
...
import React, { useState, useEffect } from 'react';
import { RotateCcw, MapPin, Clock, ThumbsUp, ThumbsDown } from 'lucide-react';
import { getIncidentsByStatus, subscribe, moveToUnverified } from '../../data/incidents';
  ...
  return (
    <div className="bg-gray-50 min-h-screen">
      <div className="container mx-auto px-6 py-8">
        <div className="mb-8">
          <h2 className="text-3xl font-bold text-gray-900 mb-2">Rejected Incidents</h2>
          <p className="text-gray-600 text-base">Incidents that were rejected. You can move them back for review.</p>
        </div>

        {incidents.length === 0 ? (
          <div className="text-center py-20 bg-white rounded-2xl border border-gray-200">
            <p className="text-gray-500 text-lg">No rejected incidents</p>
          </div>
        ) : (
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {incidents.map(incident => (
              <div key={incident.id} className="bg-white rounded-2xl border border-gray-200 p-6 hover:shadow-lg transition-shadow duration-200">
                ...
                <button
                  onClick={() => handleMoveToUnverified(incident.id)}
                  className="w-full flex items-center justify-center gap-2 bg-amber-600 hover:bg-amber-700 text-white font-semibold py-3 px-4 rounded-xl transition-colors duration-200"
                >
                  <RotateCcw size={18} />
                  <span>Move to Unverified</span>
                </button>
              </div>
            ))}
          </div>
