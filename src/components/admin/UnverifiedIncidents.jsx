...
const UnverifiedIncidents = ({ onStatusChange }) => {
  ...
  // 🟢 1. FETCH UNVERIFIED INCIDENTS
  // This calls your updated backend aggregation that provides:
  // upvoteCount, downvoteCount, and severityScore sorting
  const fetchIncidents = async () => {
    try {
      setLoading(true);
      const token = localStorage.getItem("token");
      ...
      // We set the incidents array which now includes the pre-calculated counts
      setIncidents(res.data.data || []);
    } catch (error) {
      console.error("Error fetching unverified incidents:", error);
    } finally {
  ...
  const updateStatus = async (id, newStatus) => {
    try {
      ...
      fetchIncidents();
      // Notify parent to refresh global stats
      if (onStatusChange) onStatusChange();
  ...
  const actions = [
    {
      label: 'Verify',
      icon: CheckCircle,
      onClick: (id) => updateStatus(id, 'Verified'),
      className: 'bg-green-500 hover:bg-green-600 text-white',
    },
    {
      label: 'Reject',
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            ...
            <p className="text-gray-500">
              Reviewing {incidents.length} reports requiring community verification
            </p>
          </div>
        </div>
